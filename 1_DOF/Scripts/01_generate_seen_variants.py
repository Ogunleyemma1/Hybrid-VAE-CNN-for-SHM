...
from signals_1dof import SDOFParams, simulate_free_vibration, make_clean_variants
...
def plot_stacked_variants(
    ...
    variant_specs = [
        ...
        ("Drift", f"{prefix}drift", "C1"),
        ("Amplitude-Upscaled", f"{prefix}amplitude_scaled", "C2"),
        ("Low-Frequency", f"{prefix}lowfreq", "C3"),
    ]
...
def main() -> None:
    # Script is in: 1_DOF/Scripts/01_generate_seen_variants.py
    # parents[0] = Scripts, parents[1] = 1_DOF
    root = Path(__file__).resolve().parents[1]

    data_dir = root / "Data" / "raw"
    fig_dir = root / "Output" / "figures" / "variants_seen"
    data_dir.mkdir(parents=True, exist_ok=True)
    fig_dir.mkdir(parents=True, exist_ok=True)

    # Generate baseline response + variants
    t, x, v, a = simulate_free_vibration(SDOFParams())
    variants = make_clean_variants(t, x, v, a)

    # Save CSV
    df = pd.DataFrame({"time": t, **variants})
    csv_path = data_dir / "1dof_seen_variants.csv"
    df.to_csv(csv_path, index=False)
    print(f"[OK] wrote {csv_path}")

    # Stacked figures with shared legend
    plot_stacked_variants(
        ...
        y_label_short="x (m)",
        out_dir=fig_dir,
        file_stem="seen_variants_displacement_stacked",
    )
    ...
    plot_stacked_variants(
        ...
        y_label_short="v (m/s)",
        out_dir=fig_dir,
        file_stem="seen_variants_velocity_stacked",
    )
    ...
    plot_stacked_variants(
        ...
        out_dir=fig_dir,
        file_stem="seen_variants_acceleration_stacked",
    )
