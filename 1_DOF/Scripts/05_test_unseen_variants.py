...
def build_unseen_window_labels(windows: np.ndarray) -> np.ndarray:
    """
    One label per window for the 4 unseen signal families:
      0 = Sinusoid, 1 = Envelope-Modulated, 2 = Triangle, 3 = Square
    ...
    if input_dim < 12:
        raise ValueError(f"Expected at least 12 channels (x/v/a × 4 families). Got {input_dim}.")

    family_indices = [
        [0, 4, 8],    # Sinusoid (original)
        [1, 5, 9],    # Envelope
        [2, 6, 10],   # Triangle
        [3, 7, 11],   # Square
    ]
...
def plot_latent_pca_by_type(mu: np.ndarray, y: np.ndarray, out_dir: Path, tab_dir: Path, stem: str) -> None:
    ...
    class_names = ["Sinusoid", "Envelope", "Triangle", "Square"]
...
def plot_stacked_reconstruction(
    ...
    # Unseen families in the CSV produced by make_unseen_variants:
    # original, envelope, triangle, square
    families = [
        ("Sinusoid",  f"{base_prefix}original", "C0"),
        ("Envelope",  f"{base_prefix}envelope", "C1"),
        ("Triangle",  f"{base_prefix}triangle", "C2"),
        ("Square",    f"{base_prefix}square",   "C3"),
    ]

    fig, axes = plt.subplots(nrows=4, ncols=1, figsize=(12, 8), sharex=True)

    for ax, (label, col, color) in zip(axes, families):
...
def main() -> None:
    ...
    csv_path = root / "Data" / "raw" / "1dof_unseen_variants.csv"
    ...
    out_fig = root / "Output" / "figures" / "reconstruction_unseen"
    out_tab = root / "Output" / "tables" / "reconstruction_unseen"
    ...
    data_norm = standardize(data, mean, std)
    ...
    # Save reconstruction series table
    col_names = df.columns[1:].tolist()
    recon_df = pd.DataFrame({"time": time})
    ...
    # Segment RMSE
    rmses = segment_rmse(data, recon_series, segment_len=segment_len)
    ...
    plot_stacked_reconstruction(
        ...
        y_label="x (m)",
        out_dir=out_fig,
        file_stem="reconstruction_unseen_displacement_stacked",
    )
    ...
    plot_stacked_reconstruction(
        ...
        y_label="v (m/s)",
        out_dir=out_fig,
        file_stem="reconstruction_unseen_velocity_stacked",
    )
    ...
    plot_stacked_reconstruction(
        ...
        y_label=r"a (m/s$^2$)",
        out_dir=out_fig,
        file_stem="reconstruction_unseen_acceleration_stacked",
    )

    # Latent PCA on unseen windows (colored by signal family)
    y_windows = build_unseen_window_labels(windows)
    mu_all, y_all = encode_latent_mu_with_labels(model, windows, y_windows, device=device, batch_size=512)
    ...
    plot_latent_pca_by_type(
        ...
        out_dir=out_fig,
        tab_dir=out_tab,
        stem="latent_pca_by_signal_family_unseen",
    )
